...
export const shoppingCartList = state =>{
  ...
  let list = [];
  console.log("购物车数据",state.shoppingCart)
  if(state.shoppingCart){
    for(let i = 0; i < state.shoppingCart.length; i++){
      console.log(state.shoppingCart[i].cartData)
      if(state.shoppingCart[i].cartData && state.shoppingCart[i].bookData){
        let data = state.shoppingCart[i].bookData;
        data.id = state.shoppingCart[i].cartData.id;
        data.userId = state.shoppingCart[i].cartData.userId;
        data.num = state.shoppingCart[i].cartData.num;
        list.push(data);
      }
    }
  }
  console.log("shoppingCartList：",list)
  return list;
}
